...
as $$
  ...
  session_data as (
    ...
    where s.athlete_id in (select id from all_athletes)
      ...
      and s.completed_at >= (select ts from month_start)
      and s.completed_at <  (select ts from month_end)
  ),
  sessions_count as (
  ...
  checkin_data as (
    ...
    where trainer_id = p_trainer_id
      and checked_at >= (select ts from month_start)
      and checked_at <  (select ts from month_end)
    group by athlete_id
  ),
  all_stats as (
    select
